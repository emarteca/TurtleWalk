...
def selfAvoidingWalk(t, x, y, rep, dist):
    ...
    x += dist * math.sin(degrees * 180 / math.pi) 
    y += dist * math.cos(degrees * 180 / math.pi)
    arrayOfPoints += [(x, y)]
    t.goto(x, y)
    (m, b) = solveLinearEquation(arrayOfPoints[0], arrayOfPoints[1]) 
    linearEquations = [(m, b)] # list of linear equations (in form (slope, y-intercept)) for all adjacent pairs of points in the walk
